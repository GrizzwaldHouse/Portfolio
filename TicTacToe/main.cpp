...
{ 
	...
	///////////////////////////////////////////////////////////////////////////////////
	  // Mutex to protect activeThreadCount 
	std::mutex countMutex;
	std::condition_variable readySignal;     // For main to wait for players to be ready
	// Condition variable for the starting gun
	std::condition_variable startingGun;
	// Condition variable for main thread to wait for completion
	std::condition_variable completionSignal;
	// Number of active player threads
	int activeThreadCount;
	// Total number of players we expect
	int totalPlayers;
	// Flag to indicate when threads can start playing
	bool canStartPlaying;

	PlayerPool() : activeThreadCount(0), totalPlayers(0), canStartPlaying(false) {}
};
...
void LogSync(LogSyncOperation operationToPerform)
{
	...
	case LogSyncOperation::Release:
		// No initialization/cleanup needed for std::mutex
		break;
	}
}
...
void PrintGameBoard(const Game *currentGame)
{
	...
	//   Do not use Log in this function, use printf and LogSync.
	//
	...
	{ 
		...
		switch (currentGame->currentGameState)
		{
			case GameState::StillPlaying:
				...
				///////////////////////////////////////////////////////////////////////////////////
				  // Notify other player and wait for our next turn
			...
			case GameState::Won:
				...
				///////////////////////////////////////////////////////////////////////////////////
				currentGame->gameCondition.notify_one();  // Add this line
				return;
			case GameState::Draw:
				///////////////////////////////////////////////////////////////////////////////////
...
void PlayerThreadEntrypoint(Player *currentPlayer)
{
	...
	Log("Player %d waiting on starting gun, active count before increment: %d\n",
		currentPlayer->id, currentPlayer->playerPool->activeThreadCount);
	///////////////////////////////////////////////////////////////////////////////////
	...
	//
	///////////////////////////////////////////////////////////////////////////////////
	 // Lock to modify thread count
	{
		std::unique_lock<std::mutex> lock(currentPlayer->playerPool->countMutex);
		currentPlayer->playerPool->activeThreadCount++;
		Log("Player %d increased active count to: %d\n",
			currentPlayer->id, currentPlayer->playerPool->activeThreadCount);
		// Signal main that this player is ready
		currentPlayer->playerPool->readySignal.notify_one();
		// Wait for the starting gun while watching the canStartPlaying flag
		currentPlayer->playerPool->startingGun.wait(lock,
			[pool = currentPlayer->playerPool]() { return pool->canStartPlaying; });
	}
		...
		// Signal completion
	{
		std::unique_lock<std::mutex> lock(currentPlayer->playerPool->countMutex);
		currentPlayer->playerPool->activeThreadCount--;
		if (currentPlayer->playerPool->activeThreadCount == 0) {
			currentPlayer->playerPool->completionSignal.notify_one();
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	///////////////////////////////////////////////////////////////////////////////////
	poolOfPlayers.totalPlayers = totalPlayerCount;
	poolOfPlayers.activeThreadCount = 0;
	poolOfPlayers.canStartPlaying = false;
	...
	Log("Starting player threads...\n");
	std::vector<std::thread> playerThreads;
	for (int i = 0; i < totalPlayerCount; i++)
	{
		playerThreads.push_back(std::thread(PlayerThreadEntrypoint, &perPlayerData[i]));
		playerThreads.back().detach();
	}

	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Wait for all players to be ready 
	///////////////////////////////////////////////////////////////////////////////////
	Log("Waiting for all players to be ready...\n");
	// Wait for all players to be ready
	{
		std::unique_lock<std::mutex> lock(poolOfPlayers.countMutex);
		poolOfPlayers.readySignal.wait(lock,
			[&poolOfPlayers]() { Log("Checking player count: %d vs total: %d\n",
				poolOfPlayers.activeThreadCount, poolOfPlayers.totalPlayers);
		return poolOfPlayers.activeThreadCount == poolOfPlayers.totalPlayers;  });
	}

	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Notify all waiting threads that they can start playing.
	///////////////////////////////////////////////////////////////////////////////////
	Log("All players ready, firing starting gun...\n");
	// Fire the starting gun
	{
		std::unique_lock<std::mutex> lock(poolOfPlayers.countMutex);
		poolOfPlayers.canStartPlaying = true;
	...
	///////////////////////////////////////////////////////////////////////////////////
	Log("Starting gun fired, waiting for completion...\n");
	// Wait for all players to complete
	{
		std::unique_lock<std::mutex> lock(poolOfPlayers.countMutex);
		poolOfPlayers.completionSignal.wait(lock,
			[&poolOfPlayers]() { return poolOfPlayers.activeThreadCount == 0; });
	}
